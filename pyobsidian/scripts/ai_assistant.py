...
def enhance_note(note_path):
    ...
    enhanced_content = generate_content(prompt, max_tokens=1000)
    
    new_content = f"{content}\n\n## AI-Enhanced Content\n\n{enhanced_content}"
    write_to_file(note_path, new_content)
    return note_path

def generate_note_ideas(topic):
...
def analyze_vault(config):
    ...
    analysis = generate_content(prompt, max_tokens=1500)
...
if __name__ == "__main__":
    ...
    enhance_note(note_path)
    print(generate_note_ideas("productivity"))
    print(f"Vault analysis: {analyze_vault(config)}")