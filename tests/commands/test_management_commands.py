...
def test_tag_management(mock_context: "MockContext") -> None:
    ...
    assert "#newtag" in mock_context.vault.notes["note1.md"].content
    assert "newtag" in mock_context.vault.notes["note1.md"].tags

    # Test removing a tag
    result = runner.invoke(tag_management_command.remove_tag, ["note1.md", "newtag"])
    assert result.exit_code == 0
    assert "#newtag" not in mock_context.vault.notes["note1.md"].content
    assert "newtag" not in mock_context.vault.notes["note1.md"].tags

    # Test replacing a tag
    note = mock_context.vault.notes["note1.md"]
    note.add_tag("oldtag")
    mock_context.vault.update_note(note.path, note.content)
    result = runner.invoke(tag_management_command.replace_tag, ["oldtag", "newtag"])
    assert result.exit_code == 0
    assert "#oldtag" not in mock_context.vault.notes["note1.md"].content
    assert "#newtag" in mock_context.vault.notes["note1.md"].content
    assert "oldtag" not in mock_context.vault.notes["note1.md"].tags
    assert "newtag" in mock_context.vault.notes["note1.md"].tags
